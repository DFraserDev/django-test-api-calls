def get_data(enemy_no):
    return repr(Bestiary.bestiary_list[enemy_no-1])
...
Magic Defense {self.magic_defense}Magic  Evasion {self.magic_evasion}'''


    def fullStat_summation(self, kill_count=1) -> int:
        gs_earnings = ((self.level + self.health + self.mana + self.attack + self.defense + self.evasion +
                        self.magic_power + self.magic_defense + self.magic_evasion + self.gil + self.experience)
                       * kill_count)
        return gs_earnings
...
# enemy_counter = 1
# for enemy in Bestiary.bestiary_list:
#
#     if isinstance(enemy.absorption_list, str):
#         print(f"{enemy_counter} {enemy.name} only has one element it can absorb.")
#     elif enemy.absorption_list is None:
#         print(f"{enemy_counter} {enemy.name} can't absorb any elements.")
#     elif isinstance(enemy.absorption_list, tuple):
#         print(f"{enemy_counter} {enemy.name} can absorb {len(enemy.absorption_list)} elements.")
#
#     enemy_counter += 1

max_exp: int = 0
most_recent_enemy = Bestiary.bestiary_list[len(Bestiary.bestiary_list) - 1]
for enemy in Bestiary.bestiary_list:
    max_exp += enemy.fullStat_summation(999)

print(f"Total Enemy Max Kills: {max_exp:,} EXP.")
print(f"The last enemy added was {len(Bestiary.bestiary_list)} - {most_recent_enemy.name}.")
# print(f"Remaining Entries to implement: {(384 - len(Bestiary.bestiary_list))}.")
print(f"{most_recent_enemy.name} gives {most_recent_enemy.fullStat_summation(1):,} EXP per kill.")
print(f"{most_recent_enemy.name} gives {most_recent_enemy.fullStat_summation(999):,} EXP at max kills.")
